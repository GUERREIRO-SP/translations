...
def multi_replace(text: str) -> str:
    for char in "{},":
...
def display_dados():
    ...
    lista = load_translations()

    # lista table head
    # tabel_head = ['id', 'Project', 'Strategy', 'Language', 'Key', 'Value', 'Exp.']
    tabel_head = ['Id', 'Project', 'Strategy', 'Language', 'Key', 'Value', 'Exp.']
...
lbl_tela = Label(frame_superior, text='Filtro Project', anchor=NW, font=('Ivy 10 bold'), bg=co1, fg=co4, relief='flat')
lbl_tela.place(x=0, y=1)

cbo_project = Combobox(frame_superior)
project_name = load_project_name()
# project_name = multi_replace(project_name)
cbo_project['values'] = project_name
cbo_project.current(0)     # Posiciona no 1º item do combo
cbo_project.grid(row=0, column=0, padx=0, pady=20, sticky=NSEW)

id_prj = load_project_id(project_name)
